...
class Trainer(object):
    ...
    def build_lr_scheduler(self):
        ...
        self.G_scheduler = lr_scheduler.CosineAnnealingLR(self.G_optimizer,
                                                          T_max=self.train_config.total_step - self.train_config.warmup_step,
        ...
        for step in range(start, self.train_config.total_step + 1):
            ...
            if step < self.train_config.warmup_step:
                ...
                if random.randint(0,1) == 0:
                    ...
                    weight_os4 = weight_os4.max(dim=1, keepdim=True)[0].repeat(1,3,1,1)
                    alpha_pred_os4 = alpha_pred_os4 * weight_os4 + alpha_pred_os8 * (1-weight_os4)
                    weight_os1 = utils.get_unknown_tensor_from_pred(alpha_pred_os4, rand_width=CONFIG.model.self_refine_width2, train_mode=True)
                    weight_os1 = weight_os1.max(dim=1, keepdim=True)[0].repeat(1,3,1,1)
                    alpha_pred_os1 = alpha_pred_os1 * weight_os1 + alpha_pred_os4 * (1-weight_os1)
            else:
                weight_os4 = utils.get_unknown_tensor_from_pred(alpha_pred_os8, rand_width=CONFIG.model.self_refine_width1, train_mode=True)
                weight_os4 = weight_os4.max(dim=1, keepdim=True)[0].repeat(1,3,1,1)
                alpha_pred_os4 = alpha_pred_os4 * weight_os4 + alpha_pred_os8 * (1-weight_os4)
            ...
            solid = utils.get_solid_tensor_from_alpha(alpha)
            weight_os1 = weight_os1[:,0:1] * solid
            weight_os4 = weight_os4[:,0:1] * solid
            weight_os8 = weight_os8[:,0:1] * solid


            """===== Calculate Loss ====="""
            if self.train_config.rec_weight > 0:
                self.loss_dict['rec'] = (
                    self.regression_loss(alpha_pred_os1, alpha, loss_type='l1', weight=weight_os1) * 2 +\
                    self.regression_loss(alpha_pred_os4, alpha, loss_type='l1', weight=weight_os4) * 1 +\
                    self.regression_loss(alpha_pred_os8, alpha, loss_type='l1', weight=weight_os8) * 1) / 5.0 * \
            ...
            if self.train_config.lap_weight > 0:
                self.loss_dict['lap'] = (
                    ...
                    self.lap_loss(logit=alpha_pred_os4, target=alpha, gauss_filter=self.gauss_filter.repeat(3,1,1,1),
                        loss_type='l1', weight=weight_os4) * 1 +\
                    self.lap_loss(logit=alpha_pred_os8, target=alpha, gauss_filter=self.gauss_filter.repeat(3,1,1,1),
                        loss_type='l1', weight=weight_os8) * 1) / 5.0 * \
                    self.train_config.lap_weight

            if self.train_config.comp_weight > 0:
                self.loss_dict['comp'] = (
                    self.ch3_composition_loss(alpha_pred_os1, fg, image, weight=weight_os1) * 2 + \
                    self.ch3_composition_loss(alpha_pred_os4, fg, image, weight=weight_os4) * 1 + \
                    self.ch3_composition_loss(alpha_pred_os8, fg, image, weight=weight_os8) * 1) / 5.0 *\
                    self.train_config.comp_weight

            if self.train_config.alpha_weight > 0:
                self.loss_dict['alpha'] = (
                    self.alpha_constraint_loss(alpha_pred_os1, self.one_tensor, weight=weight_os1) * 2 + \
                    self.alpha_constraint_loss(alpha_pred_os4, self.one_tensor, weight=weight_os4) * 1 + \
                    self.alpha_constraint_loss(alpha_pred_os8, self.one_tensor, weight=weight_os8) * 1) / 5.0 *\
                    self.train_config.alpha_weight

            for loss_key in self.loss_dict.keys():
                if self.loss_dict[loss_key] is not None and loss_key in [
                        'rec', 'comp', 'lap', 'alpha', 'sparse']:
