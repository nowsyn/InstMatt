...
class ToTensorTrain(object):
    ...
    def __call__(self, sample):
        ...
        sample['alpha'][-1] = 1 - sample['alpha'][:-1].sum(dim=0)
        sample['mask'] = torch.from_numpy(np.clip(mask.transpose((2,0,1)),0,1)).float()

        h, w = image.shape[1:3]
        fg = sample['fg'].reshape((h,w,3,3))[...,::-1].transpose((2,3,0,1))
...
class GenMaskTrain(object):
    ...
    def __call__(self, sample):
        ...
        fg, alpha, mask = self.combine(sample['fg'], sample['alpha'], sample['mask'])

        sample['fg'] = np.concatenate([fg, sample['bg']], axis=2) # hw9
        sample['alpha'] = np.concatenate([alpha, alpha_bg[...,None]], axis=2) # hw3
        sample['mask'] = np.concatenate([mask, mask_bg[...,None]], axis=2) # hw3

        # mask augmentation
        sample['mask'] = np.stack([self.generate(sample['mask'][...,i]) for i in range(sample['mask'].shape[2])], axis=2)
...
class CustomDataGenerator(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == "train":
            c, h, w = sample['alpha'].shape
            sample['weight'] = torch.logical_and(sample['alpha']>0.01, sample['alpha']<0.99).float().sum() / (c*h*w)
